...
export function seedNexoraDatabase(): void {
  ...
  sites.forEach(s => db.sites.set(s.id, s));
  ...
  groups.forEach(g => db.deviceGroups.set(g.id, g));

  // 4. Device Types with Dynamic Schemas
  const typeSoil: DeviceType = {
  ...
  [typeSoil, typeWater, typeMeter].forEach(dt => db.deviceTypes.set(dt.id, dt));

  // 5. Firmware Catalog
  const firmwares: FirmwareVersion[] = [
    {
      id: 'fw-soil-241',
  ...
  firmwares.forEach(fw => db.firmwareVersions.set(fw.id, fw));
  ...
  seedUsers.forEach(u => db.users.set(u.id, u));

  // 7. Anomaly ML Models
  const anomalyModels: AnomalyModelInfo[] = [
    {
  ...
  anomalyModels.forEach(m => db.anomalyModels.set(m.id, m));
  ...
  for (const cfg of deviceConfigs) {
    for (let i = 1; i <= cfg.count; i++) {
      ...
      let lifecycleState: Device['lifecycleState'] = 'ACTIVE';
  ...
  db.appendAuditLog({
    ...
    details: {
      totalDevices: db.devices.size,
      totalSites: db.sites.size,
      rootCaFingerprint: certManager.getCRL().length
    }
  });

  console.log(`[SEED] Successfully provisioned ${db.devices.size} devices across ${db.sites.size} sites with X.509 PKI credentials.`);
}

function generateInitialTelemetryHistory(device: Device, deviceType: DeviceType): void {
  const count = 20;
  const now = Date.now();
