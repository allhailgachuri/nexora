...
exports.Tier2AnomalyEngine = void 0;
const database_1 = require("../db/database");
const modelRegistry_1 = require("./modelRegistry");
const shapExplainer_1 = require("./shapExplainer");
class Tier2AnomalyEngine {
    /**
     * Evaluates multivariate sensor behavior and temporal fleet deviations
     */
    static evaluate(device, reading, recentHistory) {
        const deviceType = database_1.db.deviceTypes.get(device.deviceTypeId);
        if (!deviceType)
            return null;
        const activeModel = modelRegistry_1.modelRegistry.getActiveModelForDeviceType(deviceType.id);
        ...
        const peerDevices = database_1.db.getAllDevices().filter(d => d.deviceTypeId === device.deviceTypeId && d.siteId === device.siteId && d.id !== device.id);
        const baselines = {};
        ...
        for (const peer of peerDevices.slice(0, 15)) {
            const peerHistory = database_1.db.getTelemetryHistory(peer.id, 5);
        ...
        const { shapValues, majorDrivers } = shapExplainer_1.ShapExplainer.computeShapValues(reading.values, baselines, deviceType.telemetrySchema);
        // Determine Severity
        let severity = 'LOW';
        ...
        let anomalyType = 'OUTLIER_DEVIATION';
        if (isDrifting) {
        ...
        database_1.db.recordAnomalyEvent(event);
        return event;
    }
}
exports.Tier2AnomalyEngine = Tier2AnomalyEngine;
